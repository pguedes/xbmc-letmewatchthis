...
def resolve(page):
  log.info("looking in page: %s"%str(page))
  mega = re.compile('src="http://www.megavideo.com/v/(.+?)"').findall(page)
  log.info("video id: %s"%str(mega))
  mega[0] = mega[0][0:8]
...
def __getPublicUrl(code, megavideocookie):
  ...
  url = "http://www.megavideo.com/xml/videolink.php?v=" + code
  if megavideocookie:
    log.debug("Megavideo cookie: '%s'" % megavideocookie)
    url = "http://www.megavideo.com/xml/videolink.php?u="+megavideocookie+"&v=" + code    
  ...
  if len(errort) <= 0:
    ...
    return "http://www" + s[0] + ".megavideo.com/files/" + __calculateFileHash(un[0], k1[0], k2[0]) + "/?.flv"
...
def __getMegavideoUserCookie(login, password):
  ...
  txheaders = {'User-Agent':'Mozilla/5.0 (Windows; U; Windows NT 5.1; en-GB; rv:1.9.0.3) Gecko/2008092417 Firefox/3.0.3',
          'Referer':'http://www.megavideo.com/?c=login'}
